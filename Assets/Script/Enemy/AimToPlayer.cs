...
public class AimToPlayer : MonoBehaviour
{
    ...
    private EnemyController enemyController;  // EnemyController 참조

    void Start()
    {
        // 같은 오브젝트에 있는 EnemyController를 가져옴
    ...
    void Update()
    {
        ...
        // 플레이어가 detectionRadius 내에 있을 때만 실행
        float distanceToPlayerSqr = (player.position - enemyBody.position).sqrMagnitude;
        bool playerInRange = distanceToPlayerSqr < enemyController.detectionRadius * enemyController.detectionRadius;
